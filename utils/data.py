...
def check_split(name, split, strict_valid_test):
    available_split = {
        ...
        'parisstreetview': ['train', 'test', 'all'],
        'paris-streetview': ['train', 'test', 'all'],
    }
...
def get_dataset(name, dataroot, img_size, split,
    ...
    elif name.lower() in ['parisstreetview', 'paris-streetview']:
        from datasets.paris_streetview import ParisStreetView, get_default_transforms
        if transforms is None:
            transforms = get_default_transforms(img_size, split)
        dataset = ParisStreetView(root=dataroot, split=split, transform=transforms)
